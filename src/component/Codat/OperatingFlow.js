...
export default function OperatingFlow(props) {
    ...
    useEffect(()=>{
        ...
        const finalUrl = `${props.endUrl}/CODAT/${props.leadId}/OperatingcashflowMonths`;
    ...
    const generateData = async () => {
        if(searchIntervals !=='' && searchMonth !=='' && searchYear !==''){
            const finalUrl = `${props.endUrl}/CODAT/getOperatingCashFlow/${props.leadId}/${searchIntervals}/${searchMonth}/${searchYear}`;
            const response = await axios.get(finalUrl);
      ...
      return (
        <div className="codatcontainer"> 
            <div className="col-md-12"><h3>Operating cash flow <br /><span>Currency GBP</span></h3></div>
            <div className="row">
